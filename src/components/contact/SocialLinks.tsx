...
const SOCIAL_DEFS: { key: string; label: string; icon: string; href: (v: string) => string }[] = [
  {
    key: 'email',
    label: 'Email',
  ...
  return (
    <div className="md:w-64 lg:w-72 flex-shrink-0">
      ...
      <div className="space-y-3">
        {links.map(def => {
          const val = social[def.key as keyof typeof social]
          const href = def.key === 'email' ? `mailto:${val}` : def.href(val)
          const display = def.key === 'email' ? val : val

          return (
            <Link
              key={def.key}
              href={href}
              target={def.key === 'email' ? undefined : '_blank'}
              rel={def.key === 'email' ? undefined : 'noopener noreferrer'}
              className="flex items-center gap-3 p-3 rounded-lg border border-[var(--border)] hover:border-[var(--accent)] bg-[var(--card)] transition-colors group"
            >
              <span
              ...
              <div className="min-w-0">
                ...
                <div className="text-xs text-[var(--muted)] truncate">{display}</div>
              </div>
              <svg className="ml-auto w-4 h-4 text-[var(--muted)] group-hover:text-[var(--accent)] opacity-0 group-hover:opacity-100 transition-all flex-shrink-0" viewBox="0 0 16 16" fill="currentColor">
