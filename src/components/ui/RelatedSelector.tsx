...
export function RelatedSelector({ type, selected, onChange }: RelatedSelectorProps) {
  ...
  useEffect(() => {
    fetch('/api/search/index')
      .then(r => r.json())
      .then((data: SearchItem[]) => setItems(data.filter(i => i.type === type)))
      .catch(() => setItems([]))
  }, [type])
  ...
  const fuse = useMemo(() => {
    ...
    return new Fuse(items, {
      keys: [{ name: 'title', weight: 0.6 }, { name: 'company', weight: 0.4 }],
      threshold: 0.4,
    })
  }, [items])

  const results: SearchItem[] = query.length >= 1
    ? (fuse?.search(query, { limit: 8 }).map(r => r.item) ?? [])
    : (items?.slice(0, 8) ?? [])

  const selectedItems = (items ?? []).filter(i => selected.includes(i.id.replace(`${type}-`, '')))
  ...
  const placeholder = type === 'project' ? 'Search projects…' : 'Search experience…'
    ...
    <div ref={containerRef} className="col-span-2 space-y-2">
      ...
      <div className="relative">
        <div className="flex items-center gap-2 bg-[var(--background)] border border-[var(--border)] rounded-lg px-3 py-2 focus-within:ring-1 focus-within:ring-[var(--accent)] transition">
          ...
          />
          {query && (
            <button type="button" onClick={() => setQuery('')} className="text-[var(--muted)] hover:text-[var(--foreground)] cursor-pointer">
            ...
            {results.map(item => {
              ...
              return (
                ...
                >
                  ...
                  {item.company && <span className="text-xs text-[var(--muted)] truncate">{item.company}</span>}
                  {isSelected && (
                    <svg className="flex-shrink-0 text-[var(--accent)]" width="14" height="14" viewBox="0 0 16 16" fill="currentColor">
