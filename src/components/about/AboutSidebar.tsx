...
const SOCIAL_DEFS: { key: string; icon: string; label: string; href: (val: string) => string }[] = [
  {
...
export function AboutSidebar() {
  ...
  const activeSocialLinks = SOCIAL_DEFS.filter(def => {
    const val = social[def.key as keyof typeof social]
    return val && val.trim() !== ''
  })
  ...
  return (
    <aside className="w-full md:w-64 lg:w-72 flex-shrink-0">
      ...
      <div className="mb-5">
        <div className="relative w-48 h-48 rounded-full overflow-hidden border-[3px] border-[var(--border)] bg-[var(--muted-bg)] shadow-lg">
          <Image
            src={site.avatar}
            alt={site.title}
            fill
      ...
      {/* Name & tagline */}
      <h1 className="text-xl font-bold text-[var(--foreground)] mb-0.5">{site.title}</h1>
      <p className="text-sm text-[var(--muted)] font-light mb-1">{site.tagline}</p>
      <p className="text-sm text-[var(--foreground)] mb-4 leading-relaxed opacity-80">{site.bio}</p>
      ...
      <ul className="space-y-2 mb-4">
        {site.location && (
          <li className="flex items-center gap-2 text-sm text-[var(--muted)]">
            <span>📍</span><span>{site.location}</span>
          </li>
        )}
        {social.email && (
          <li className="flex items-center gap-2 text-sm text-[var(--muted)]">
            <span>✉️</span>
            <a href={`mailto:${social.email}`} className="hover:text-[var(--accent)] transition-colors">{social.email}</a>
          </li>
        )}
      </ul>
          ...
          <div className="flex flex-col gap-2 mb-4">
            {activeSocialLinks.map(def => {
              ...
              return (
                ...
                >
                  <span className="w-4 h-4 flex-shrink-0" dangerouslySetInnerHTML={{ __html: def.icon }} />
                  <span>{def.label}</span>
                  <span className="text-xs opacity-50 truncate">{val}</span>
                </Link>
              )
            })}
          </div>
            ...
            <div key={cat.category}>
              ...
              <div className="flex flex-wrap gap-1.5">
                {cat.items.map((skill: string) => (
                  <span
                    key={skill}
                    className="text-xs px-2 py-0.5 rounded-full bg-[var(--muted-bg)] text-[var(--foreground)] border border-[var(--border)] hover:border-[var(--accent)] transition-colors"
                  >
                    {skill}
                  </span>
                ))}
