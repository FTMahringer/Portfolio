...
import type { HomepageStatComputedKey, HomepageStatItem } from '@/lib/site-settings-types';
...
export default function HomepageSettingsPage() {
  ...
  return (
    <SettingsFormShell title={t('settings.pages.homepage.title')} description={t('settings.pages.homepage.description')}>
      {({ settings, setSettings, save, saving }) => (
        <div className="space-y-6">
          <Card title={t('settings.pages.homepage.sections.stats.title')}>
            <div className="space-y-4">
              <label className="flex items-center justify-between gap-4 rounded-xl border border-[var(--border)] bg-[var(--background)] px-4 py-3 text-sm text-[var(--foreground)]">
                <span>{t('settings.pages.homepage.sections.stats.enabled')}</span>
                <input
                  type="checkbox"
              ...
              >
                {t('settings.pages.homepage.actions.addStat')}
              </button>
            </div>
          </Card>
    ...
    <div className="rounded-2xl border border-[var(--border)] bg-[var(--background)] p-4">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
          <SmallButton onClick={() => moveStat(setSettings, index, -1)}>{t('settings.pages.homepage.actions.moveUp')}</SmallButton>
          <SmallButton onClick={() => moveStat(setSettings, index, 1)}>{t('settings.pages.homepage.actions.moveDown')}</SmallButton>
          <SmallButton danger onClick={() => removeStat(setSettings, index)}>{t('settings.pages.homepage.actions.remove')}</SmallButton>
        </div>
      </div>
      <div className="grid gap-4 md:grid-cols-2">
        <Field label={t('settings.pages.homepage.fields.label')}>
          <input value={item.label} onChange={(event) => updateStat(setSettings, index, { label: event.target.value })} className={inputClass} />
        </Field>
        <Field label={t('settings.pages.homepage.fields.suffix')}>
          <input value={item.suffix} onChange={(event) => updateStat(setSettings, index, { suffix: event.target.value })} className={inputClass} />
        </Field>
        <Field label={t('settings.pages.homepage.fields.source')}>
          <select value={item.source} onChange={(event) => updateStat(setSettings, index, { source: event.target.value as 'computed' | 'manual' })} className={inputClass}>
            <option value="computed">{t('settings.pages.homepage.sources.computed')}</option>
            <option value="manual">{t('settings.pages.homepage.sources.manual')}</option>
          </select>
        </Field>
        {item.source === 'computed' ? (
          <Field label={t('settings.pages.homepage.fields.computedKey')}>
            <select value={item.computedKey ?? 'totalProjects'} onChange={(event) => updateStat(setSettings, index, { computedKey: event.target.value as HomepageStatComputedKey })} className={inputClass}>
              {COMPUTED_KEYS.map((key) => <option key={key} value={key}>{t(`settings.pages.homepage.computedKeys.${key}`)}</option>)}
            </select>
          </Field>
        ) : (
          <Field label={t('settings.pages.homepage.fields.manualValue')}>
        ...
        <label className="flex items-center justify-between gap-4 rounded-xl border border-[var(--border)] px-4 py-3 text-sm text-[var(--foreground)] md:col-span-2">
          <span>{t('settings.pages.homepage.fields.enabled')}</span>
