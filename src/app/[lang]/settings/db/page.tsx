...
type QueryResult = { columns: string[]; rows: Record<string, unknown>[]; rowCount: number; message?: string; error?: string }
...
export default function DevDbPage() {
  ...
  const runQuery = useCallback(async (q?: string) => {
    ...
    try {
      ...
      setResult({ columns: [], rows: [], rowCount: 0, error: t('settings.pages.db.errors.network') })
    }
    setRunning(false)
  }, [query, t])
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-8 pt-8 pb-4 flex-shrink-0">
        <h1 className="text-2xl font-bold text-[var(--foreground)] mb-0.5">{t('settings.pages.db.title')}</h1>
        <p className="text-sm text-[var(--muted)]">{t('settings.pages.db.description')}</p>
      </div>

      <div className="flex flex-1 overflow-hidden px-8 pb-8 gap-5">
        <aside className="w-44 flex-shrink-0 flex flex-col gap-1">
          <p className="text-[10px] font-semibold uppercase tracking-widest text-[var(--muted)] mb-1.5">{t('settings.pages.db.tables')}</p>
          {tables.length === 0 && (
            <p className="text-xs text-[var(--muted)] italic">{t('settings.pages.db.loading')}</p>
          )}
          {tables.map(tMeta => (
            <button
              key={tMeta.name}
              onClick={() => selectTable(tMeta.name)}
              className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-colors cursor-pointer ${
                activeTable === tMeta.name
                  ? 'bg-green-400/10 text-green-400 font-medium'
                  : 'text-[var(--muted)] hover:text-[var(--foreground)] hover:bg-[var(--muted-bg)]'
              }`}
            >
              <span className="font-mono truncate">{tMeta.name}</span>
              <span className="text-xs opacity-60 flex-shrink-0 ml-1">{tMeta.count}</span>
            </button>
          ))}
        </aside>
        ...
        <div className="flex-1 flex flex-col gap-4 min-w-0">
          <div className="rounded-xl border border-[var(--border)] overflow-hidden flex-shrink-0">
            <div className="flex items-center justify-between px-4 py-2 border-b border-[var(--border)] bg-[var(--card)]">
              <span className="text-[10px] font-mono font-semibold uppercase tracking-widest text-[var(--muted)]">{t('settings.pages.db.editor.title')}</span>
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-[var(--muted)]">{t('settings.pages.db.editor.hint')}</span>
                <button
                  onClick={() => runQuery()}
                  disabled={running}
                ...
                >
                  ...
                  {t('settings.pages.db.actions.run')}
                </button>
              </div>
            </div>
            <textarea
            ...
            <div className="rounded-xl border border-[var(--border)] overflow-hidden flex-1 min-h-0 flex flex-col">
              <div className="flex items-center justify-between px-4 py-2 border-b border-[var(--border)] bg-[var(--card)] flex-shrink-0">
                <span className="text-[10px] font-mono font-semibold uppercase tracking-widest text-[var(--muted)]">
                  {t('settings.pages.db.results.title')}
                </span>
                {!result.error && (
                  <span className="text-xs text-[var(--muted)]">
                    {result.message ?? `${result.rowCount} ${result.rowCount === 1 ? t('settings.pages.db.results.rowSingle') : t('settings.pages.db.results.rowPlural')}`}
                  </span>
                )}
              </div>
                    ...
                    <tbody>
                      {result.rows.map((row, i) => (
                        <tr key={i} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--muted-bg)]">
                          {result.columns.map(col => {
                            const val = row[col]
                            const display = val === null ? <span className="text-[var(--muted)] italic">{t('settings.pages.db.results.null')}</span>
