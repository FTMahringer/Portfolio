...
export async function generateMetadata({ params }: PageParams): Promise<Metadata> {
  ...
  return {
    title: tr(t, 'public.homelab.meta.title', 'Homelab'),
    description: tr(t, 'public.homelab.meta.description', 'Homelab content coming soon.'),
  };
}

export default async function HomelabPage({ params }: PageParams) {
  const { lang } = await params;
  const t = await getTranslations(lang);
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 sm:px-6 py-16">
      <h1 className="text-3xl font-bold mb-3 text-[var(--foreground)]">
        {tr(t, 'public.homelab.heading', 'Homelab')}
      </h1>
      ...
      {content ? (
        ...
        <p className="text-[var(--muted)]">
          {tr(t, 'public.homelab.fallback', 'Homelab content coming soon.')}
        </p>
      )}
    </main>
  );
