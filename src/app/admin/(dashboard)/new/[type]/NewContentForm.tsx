          ...
          {mode === "month" && (
            <div className="grid grid-cols-3 gap-1.5 p-3">
              {SHORT_MONTHS.map((m, i) => {
                const isSelected =
                  parsed && parsed.year === year && parsed.month === i;
                return (
                  <button
                    key={m}
                    type="button"
              ...
              <div className="px-3 pt-2">
                <div className="grid grid-cols-7 mb-1">
                  {DAY_LABELS.map((d) => (
                    <div
                      key={d}
                      className="text-center text-[10px] font-bold text-[var(--muted)] py-1"
                    >
                      {d}
                    </div>
                  ))}
...
function Wizard({
  ...
  return (
    <div
      className={
        className ??
        "bg-[var(--background)] text-[var(--foreground)] flex flex-col"
      }
    >
      {/* Step progress bar */}
    ...
    return (
      <div className="space-y-6">
        <ImageUpload
          ...
          onAdd={(imgs) =>
            set("images", [...(form.images as string[]), ...imgs])
          }
          onRemove={(img) =>
            set(
              "images",
              (form.images as string[]).filter((i) => i !== img),
            )
          }
        />
        <RelatedPicker
        ...
        <FieldWrapper>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              set(
                "slug",
                slugify((form.company as string) + "-" + e.target.value),
              );
            }}
          />
        </FieldWrapper>
        <FieldWrapper>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              set(
                "slug",
                slugify(e.target.value + "-" + (form.title as string)),
              );
            }}
          />
        </FieldWrapper>
      ...
      <Grid>
        ...
        <FieldWrapper>
          ...
          <DatePicker
            ...
            placeholder={
              (form.present as boolean) ? "Present" : "Pick end month…"
            }
...
export default function NewContentForm({ type }: { type: FormType }) {
  ...
  async function handleSubmit() {
    ...
    for (const k of ["images", "relatedProjects", "relatedExperience"]) {
      if (
        Array.isArray(frontmatter[k]) &&
        !(frontmatter[k] as unknown[]).length
      ) {
  ...
  return (
    <div className="min-h-screen bg-[var(--background)]">
      ...
      <Wizard
        ...
        onNext={() => {
          if (canNext) {
            ...
            setError(
              `Please fill in: ${requiredForStep.filter((k) => !form[k]).join(", ")}`,
            );
          }
        }}
      ...
      >
        ...
        {type === "project" && (
          <ProjectSteps step={step} form={form} set={set} />
        )}
        {type === "experience" && (
          <ExperienceSteps step={step} form={form} set={set} />
        )}
      </Wizard>
    </div>
  );
