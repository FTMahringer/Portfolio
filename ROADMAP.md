...
- **Project image gallery**: Hero image with clickable thumbnail strip below, smooth transitions
- **Analytics support**: Umami and Plausible integration (opt-in, production-only)
- **Content validation**: Script to validate MDX frontmatter against TypeScript types
...
- `config/site.yaml`: central config for name, title, contact links, social URLs, giscus settings, analytics

---

## Future Ideas

See `docs/ideas/` for organized feature ideas:
- `completed.md` — Features we've built
- `planned.md` — Features we're actively considering
- `maybe.md` — Possible future enhancements
- `unnecessary.md` — Features marked as overkill (for reference)
